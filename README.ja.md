...
レビュアーが受け入れ／却下するのはアップグレード 1 件なので、リクエスト 1 本に
**パッケージ 1 つ**を載せます（そのパッケージの全インストール済みバージョンを含む）。
同じ依存が複数のマニフェストに別バージョンで入っていることが多く、それを別リクエストに
分けると、同じ lockfile を複数の Agent が同時に再生成してしまいます。全部まとめていた頃は、あるリポジトリで lockfile の差分が 17,000 行になり、
